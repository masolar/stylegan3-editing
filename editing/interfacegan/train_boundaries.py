...
def main(opts: TrainConfig):
    ...
    for batch_dir in tqdm(opts.input_path.glob("*")):
        ...
        if not str(batch_dir.name).startswith("id_"):
            continue
        # load batch latents
        latent_codes = np.load(
            opts.input_path / batch_dir / 'ws.npy', allow_pickle=True)
        all_latent_codes.extend(latent_codes.tolist())
        # load batch attribute scores
        scores = np.load(opts.input_path / batch_dir /
                         'scores.npy', allow_pickle=True)
        all_attribute_scores.extend(scores.tolist())
        # load batch ages
        ages = np.load(opts.input_path / batch_dir /
                       'ages.npy', allow_pickle=True)
        all_ages.extend(ages.tolist())
        # load batch poses
        poses = np.load(opts.input_path / batch_dir /
                        'poses.npy', allow_pickle=True)
    ...
    for attribute_name in attr_list:
        ...
        attr_scores = [s[attr_list.index(attribute_name)][1]
                       for s in all_attribute_scores]
