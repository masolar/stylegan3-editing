...
class VideoHandler:
    ...
    def __init__(self, video_path: Path, output_path: Path, raw_frames_path: Path = None,
        ...
        self.raw_frames_path = output_path / \
            "raw_frames" if raw_frames_path is None else raw_frames_path
        self.aligned_frames_path = output_path / \
            "aligned_frames" if aligned_frames_path is None else aligned_frames_path
        self.cropped_frames_path = output_path / \
            "cropped_frames" if cropped_frames_path is None else cropped_frames_path
    ...
    def parse_video(self):
        ...
        if len(list(self.raw_frames_path.glob("*"))) == 0:
            ...
            frames_paths = [self.raw_frames_path /
                            f for f in self.raw_frames_path.iterdir()]
        # get aligned video frames
        if len(list(self.aligned_frames_path.glob("*"))) == 0:
            self._save_aligned_video_frames(frames_paths)
        else:
            print(
                f"Aligned video frames already saved to: {self.aligned_frames_path}")
        # get all cropped video frames
        if len(list(self.cropped_frames_path.glob("*"))) == 0:
            self._save_cropped_video_frames(frames_paths)
        else:
            print(
                f"Cropped video frames already saved to: {self.cropped_frames_path}")

    def get_input_paths(self):
        sorted_paths = sorted(self.aligned_frames_path.iterdir(
        ), key=lambda x: int(str(x.name).replace(".jpg", "")))
        file_names = [f.name for f in sorted_paths]
        aligned_paths = [self.aligned_frames_path /
                         file_name for file_name in file_names]
        cropped_paths = [self.cropped_frames_path /
                         file_name for file_name in file_names]
        return aligned_paths, cropped_paths

    @staticmethod
    def load_images(input_paths: List[Path]):
        input_images = [Image.open(input_path).convert("RGB")
    ...
    def _save_aligned_video_frames(self, frames_paths: List[Path]):
        ...
        for path in tqdm(frames_paths):
            try:
                im = align_face(filepath=str(path), detector=detector,
                                predictor=predictor).convert("RGB")
    ...
    def _save_cropped_video_frames(self, frames_paths: List[Path]):
        ...
        c, x, y = get_alignment_positions(
            str(frames_paths[0]), detector, predictor)
